...
**Status: All 7 steps done** (Foundations, Auth, Catalog browsing, Borrowing, Profile & personal library, E-library reading experience, Polish). See [`web-app-Steps.md`](web-app-Steps.md) for the step-by-step build checklist — one step, one branch, same discipline as `Server/app/docs/Steps.md`. This file covers the *why* and the cross-cutting architecture decisions; the Steps file covers the *what*, in order.
